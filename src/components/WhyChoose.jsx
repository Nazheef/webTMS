    ...
    <section id="why-choose" className="py-20 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl  text-center text-gray-800 mb-16           text-4xl font-bold text-gray-800 dark:text-white"  >
         {t.whyChoose .split(' ').map((word, i) =>
                  word === "" ? (
                    <span key={i} className="text-yellow-500 font-style-italic">{word}</span>
                  ) : (
                    `${word} `
                  )
                )} <span className="text-yellow-500">Treemas Solusi Utama</span>? 
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { title: t.quick, time:  t.minutesfive, steps: [t.Createyouraccount, t.Addcompanyinformation, t.Setuserpermissions, t.Importexistingdata] },
            { title: t.Configure, time:  t.minutefifteen, steps: [t.Configurechartofaccounts, t.Setupapprovalworkflows, t.Definebudgetcategories, t.Connectbankaccounts] },
            { title: t.monitor, time:  t.ongoing, steps: [t.Viewfinancialdashboards, t.Generatercustomreports, t.Trackkeymetrics, t.Setupautomatedalerts] },
            { title: t.Scale, time:  t.asneeded, steps: [t.Addnewmodules, t.Integratewithothersystems, t.Accessadvancedanalytics, t.Getpremiumsupport] }
          ].map((step, i) => (
